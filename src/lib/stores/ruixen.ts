...
class Ruixen {
  ...
  private offlineHeuristic(pet: PetPanelData, entry: JournalEntry): AnalysisResult {
    ...
    const signsConcerning = /(letharg|not eat|didn\'t eat|no appetite|vomit|diarrhea|labored|wheeze|injur|blood|hiding|corner|less singing|cough)/i.test(text);
    ...
    if (signsConcerning) {
      if ((pet.species || '').toLowerCase().includes('bird') || (pet.breed || '').toLowerCase().includes('cockatiel')) {
  ...
  weeklySummary(pet: PetPanelData) {
    ...
    const appetiteHints = last7.filter((e) => /eat|appetite|food|breakfast|dinner/i.test(e.content)).length;
    const socialHints = last7.filter((e) => /play|walk|cuddle|sing|interaction|social/i.test(e.content)).length;
    ...
    return {
      ...
      recommendation: socialHints < Math.ceil(last7.length / 3) ? 'Increase enrichment and interaction time' : 'Keep routine',
    };
  }
  ...
  speciesInsights(pet: PetPanelData) {
    ...
    if (species.includes('parrot') || species.includes('budgie') || (pet.species || '').toLowerCase() === 'bird') {
      return [
  ...
  scheduleReminders(pet: PetPanelData) {
    ...
    return [
      ...
      { title: 'Diet review', due: dietDue, note: dietDue ? 'Add leafy greens / adjust portions' : 'OK' },
    ];
  }
}
