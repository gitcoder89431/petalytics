<script lang="ts">
	...
	import { uiHelpers, createPetFormOpen } from '$lib/stores/ui';
	...
	// (archive is handled in right panel via confirmArchive view)
	...
	function handleAutocomplete(
		field: 'species' | 'breed' | 'ageUnit' | 'gender' | 'size',
		suggestions: readonly string[] | string[],
		e: KeyboardEvent
	) {
		if (e.key === 'Enter') {
	...
	// unarchive is available via Data Manager; no direct button here
...
<div class="pet-panel h-full" style="background: var(--petalytics-bg);">
	...
	>
		...
		<div
			...
			onkeydown={(e) => handleActivate(e, toggleCreateForm)}
		>
		...
		{#if showCreateForm}
			<div class="mt-2 p-2 rounded" style="background: var(--petalytics-overlay);">
				...
				<div class="cli-row px-2 py-1">
					...
					<input
						class="value bg-transparent border-none outline-none input-inline"
				...
				<div class="cli-row px-2 py-1">
					...
					<input
						...
						oninput={onSpeciesInput}
						onkeydown={(e) => handleAutocomplete('species', speciesSuggestions, e)}
					/>
				</div>
				...
				<div class="cli-row px-2 py-1">
					...
					<input
						...
						onkeydown={(e) =>
							handleAutocomplete('breed', getBreedSuggestions(norm(newPet.species)), e)}
					/>
				</div>
				...
				<div class="cli-row px-2 py-1">
					...
					<div class="value flex items-center justify-end gap-2">
						<input
							class="bg-transparent border-none outline-none input-inline w-20 text-right"
							type="number"
							min="0"
						...
						/>
						<input
							class="bg-transparent border-none outline-none input-inline w-28"
							bind:value={newPet.ageUnit}
							placeholder="years|months|weeks"
							list="age-unit-suggestions"
							onkeydown={(e) => handleAutocomplete('ageUnit', ageUnitSuggestions, e)}
						/>
					</div>
				</div>
				...
				<div class="cli-row px-2 py-1">
					...
					<input
						...
						onkeydown={(e) => handleAutocomplete('gender', genderSuggestions, e)}
					/>
				</div>
				...
				<div class="cli-row px-2 py-1">
					...
					<input
						class="value bg-transparent border-none outline-none input-inline"
						bind:value={newPet.size}
						placeholder="tiny|small|medium|large|extra_large"
						list="size-suggestions"
						onkeydown={(e) => handleAutocomplete('size', sizeSuggestions, e)}
					/>
				</div>
				...
				<div class="cli-row px-2 py-1" style="align-items: flex-start;">
					...
					<div class="value flex items-center justify-end gap-2">
						{#if newPet.profileImageUrl}
							...
							/>
						{/if}
						<button type="button" class="arrow-btn" onclick={() => imageInput.click()}
							>upload</button
						>
						<input
				...
				<div class="flex gap-2 px-2 pt-2">
					...
					<button class="button-secondary flex-1" type="button" onclick={toggleCreateForm}
						>cancel</button
					>
				</div>
			</div>
		...
		{#if activePets.length === 0}
			<div class="px-2 py-4" style="color: var(--petalytics-subtle);">
				no pets yet — use add_pet to create one
			</div>
		{:else}
			{#each activePets as pet}
				<div
					...
					role="button"
					tabindex="0"
					data-selected={selectedPetId === pet.id}
					onclick={() => selectPet(pet.id)}
					onkeydown={(e) => handleActivate(e, () => selectPet(pet.id))}
				>
					<span class="label" style="color: var(--petalytics-text);">{pet.name}</span>
					<span class="value" style="color: var(--petalytics-subtle);">
						{pet.species || 'pet'} | {pet.breed || '—'} | {pet.age}{pet.ageUnit === 'months'
							? 'm'
							: pet.ageUnit === 'weeks'
								? 'w'
								: 'y'}
		...
		<div
			class="cli-row px-2 py-1 items-center"
			style="background: color-mix(in oklab, var(--petalytics-overlay) 60%, transparent);"
		>
			<span style="color: var(--petalytics-subtle);">#</span>
			...
			<button
				class="arrow-btn"
				onclick={() => openMemorial('all')}
				disabled={archivedPets.length === 0}>view_memories</button
			>
		</div>
