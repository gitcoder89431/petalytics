...
export class DataExporter {
  ...
  static async exportAllData(): Promise<void> {
    try {
      ...
      const jsonlContent = Object.keys(exportData).map(key => 
        JSON.stringify({ [key]: exportData[key as keyof FullDataExport] })
      ).join('\n');
  ...
  private static async importFullBackup(data: FullDataExport): Promise<{ success: boolean; message: string }> {
    let count = 0;
